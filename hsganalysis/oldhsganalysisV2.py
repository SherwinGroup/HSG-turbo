...
This version should work with spectra files 
which were created pre-live sequencing
(Early Nov '15)
"""
...
def stitch_hsg_dicts(full, new_dict, need_ratio=False, verbose=False):
    ...
    if need_ratio:
        print('\n2024\nfull[2]', full[0][2])
    for sb in [x for x in list(new_dict.keys()) if (x >= new_starter)]:
        full[sb] = new_dict[sb]
        if need_ratio:
